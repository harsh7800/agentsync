...
export class ManualScanController {
     ...
     * @returns Scan result
     */
    async scanWithUserOptions(options) {
        ...
        if (options.includePatterns && options.includePatterns.length > 0) {
            scanOptions.patterns = options.includePatterns;
        }
        // Perform the scan
     ...
     */
    async calculateDirectoryDepth(dir, currentDepth, maxDepth) {
        ...
        try {
            ...
            for (const entry of entries) {
                if (entry.isDirectory() && !entry.name.startsWith('.') && entry.name !== 'node_modules') {
                    const subDepth = await this.calculateDirectoryDepth(join(dir, entry.name), currentDepth + 1, maxDepth);
     ...
     */
    applyExcludePatterns(agents, excludePatterns) {
...
//# sourceMappingURL=manual-scan.js.map