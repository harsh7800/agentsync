...
export class Scanner {
    ...
    getScanPaths(scope, cwd) {
        ...
        if (scope === 'system' || scope === 'both') {
            ...
            paths.push(join(homedir(), '.config'), join(homedir(), '.cursor'));
        }
        return paths;
    }
    /**
     ...
     */
    async scanPath(basePath, tools, maxDepth, cwd) {
        const agents = [];
     ...
     */
    async findFilesInDir(dir, patterns, maxDepth, currentDepth = 0) {
        const files = [];
     ...
     */
    async scanWithPattern(pattern, cwd, maxDepth) {
     ...
     */
    async createAgentFromFile(filePath, tool) {
        try {
