...
import { isToolSupported, getSupportedTools, getToolPatterns, DEFAULT_TOOL_PATTERNS } from './patterns.js';
...
export class AIAssistedScanner {
  ...
  private cache: Map<string, AIAssistedScanResult> = new Map();
   ...
   */
  async scan(options: AIAssistedScanOptions): Promise<AIAssistedScanResult> {
   ...
   */
  private async enhanceWithContentAnalysis(agent: DetectedAgent): Promise<void> {
    try {
   ...
   */
  private async calculateComplexity(agent: DetectedAgent): Promise<void> {
    try {
   ...
   */
  private learnPatterns(agents: DetectedAgent[]): string[] {
   ...
   */
  private generateMigrationSuggestions(agents: DetectedAgent[]): MigrationSuggestion[] {
   ...
   */
  private buildCompatibilityMatrix(agents: DetectedAgent[]): CompatibilityMatrix {
   ...
   */
  private groupAgents(agents: DetectedAgent[]): AgentGroup[] {
    const groups: AgentGroup[] = [];
    
    // Group by tool
    const byTool = new Map<string, DetectedAgent[]>();
    for (const agent of agents) {
   ...
   */
  private estimateMigrationEffort(agents: DetectedAgent[]): MigrationEstimate {
   ...
   */
  private recommendTargets(agents: DetectedAgent[]): string[] {
   ...
   */
  private detectConflicts(agents: DetectedAgent[]): ConflictInfo[] {
   ...
   */
  private async detectOutdatedConfigs(agents: DetectedAgent[]): Promise<string[]> {
   ...
   */
  private generateCacheKey(options: AIAssistedScanOptions): string {
