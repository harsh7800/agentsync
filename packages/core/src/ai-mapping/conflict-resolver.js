...
export class ConflictResolver {
     ...
     */
    detectConflicts(mappings, targetFields) {
        const conflicts = [];
     ...
     */
    resolveConflicts(mappings, strategy = 'strict', targetFields) {
        const conflicts = this.detectConflicts(mappings, targetFields);
        const resolvedMappings = [];
        const unresolvedConflicts = [];
        if (strategy === 'strict') {
            // In strict mode, only keep non-conflicting mappings
            const sourceToBestTarget = new Map();
            const targetToBestSource = new Map();
