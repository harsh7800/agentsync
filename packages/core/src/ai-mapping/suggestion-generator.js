...
export class SuggestionGenerator {
     ...
     */
    suggestBasedOnPatterns(sourceFields, targetFields) {
        const suggestions = [];
     ...
     */
    generateSemanticSuggestions(source, target) {
        const suggestions = [];
     ...
     */
    suggestTypeCompatibleMappings(source, target) {
        const suggestions = [];
        for (const sourceField of source.fields) {
            for (const targetField of target.fields) {
     ...
     */
    calculateDescriptionSimilarity(sourceField, targetField) {
     ...
     */
    calculateAbbreviationSimilarity(field1, field2) {
     ...
     */
    generatePrioritizedSuggestions(source, target) {
        const suggestions = this.suggestTypeCompatibleMappings(source, target);
        // Boost confidence for required field mappings
        const targetRequiredFields = new Set(target.fields.filter(f => f.required).map(f => f.name));
     ...
     */
    suggestWithWeightedScoring(sourceFields, targetFields) {
        const suggestions = [];
        for (const sourceField of sourceFields) {
     ...
     */
    scoreByUsageFrequency(analysis, frequentlyUsedFields) {
     ...
     */
    calculateContextSimilarity(sourceContext, targetContext) {
     ...
     */
    suggestHierarchicalMappings(source, target) {
        const suggestions = [];
        for (const sourceField of source.fields) {
            for (const targetField of target.fields) {
                ...
                if (sourceField.nested && targetField.nested) {
                    const nestedSuggestions = this.suggestNestedMappings(sourceField.nested, targetField.nested, sourceField.name, targetField.name);
     ...
     */
    suggestArrayMappings(source, target) {
        const suggestions = [];
     ...
     */
    suggestNestedPathMappings(sourceFields, targetFields) {
        const suggestions = [];
     ...
     */
    suggestWithFallbacks(sourceFields, targetFields) {
     ...
     */
    suggestDefaultValues(source, target) {
        const defaults = [];
     ...
     */
    generateAlternatives(sourceField, targetFields) {
        const alternatives = [];
        for (const targetField of targetFields) {
     ...
     */
    calibrateConfidence(historicalData) {
     ...
     */
    calculateCalibratedConfidence(source, target, baseScore) {
     ...
     */
    adjustForAmbiguity(source, target, baseScore) {
        // Reduce confidence for common/generic field names
        const ambiguousTerms = ['data', 'info', 'value', 'item', 'config'];
        const isAmbiguous = ambiguousTerms.some(term => source.toLowerCase().includes(term) || target.toLowerCase().includes(term));
    ...
    // Helper methods
    findBestPatternMatch(field, candidates) {
    ...
    calculateSemanticSimilarity(sourceField, targetField) {
    ...
    suggestNestedMappings(sourceFields, targetFields, parentSource, parentTarget) {
        const suggestions = [];
        for (const sourceField of sourceFields) {
            for (const targetField of targetFields) {
                const score = this.calculator.calculateSimilarity(sourceField.name, targetField.name);
